...
internal class WorkQueryTools(private val reader: WorkItemReader) {
    ...
    private fun registerGet(mcp: Server) {
        mcp.addTool(
            ...
            val raw = McpTooling.arg(request.arguments, "id").orEmpty()
            val parsed = McpTooling.parseId(raw)
            McpTooling.text(
                when {
                    parsed == null -> WorkToolJson.invalidArgument("id manquant ou invalide : '$raw'")
    ...
    private fun registerListActive(mcp: Server) {
        mcp.addTool(
            ...
            McpTooling.text(WorkToolJson.activeItems(reader.active()))
        }
    }
}
